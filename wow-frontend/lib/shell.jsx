      ...
      <div className="wow-sidebar-foot">
        <div className="wow-agent-card">
          ...
          <div className="wow-agent-info">
            ...
            <div className="wow-agent-status">● Live · 7 active</div>
          </div>
        </div>
...
// ============ ORB CONSOLE ============
function OrbConsole({ orbSrc, live, dialing, onStart, onStop }) {
    ...
    <div className="wow-card wow-orb-console">
      ...
      <div className="wow-orb-side">
        ...
        <div className="wow-orb-actions">
          ...
          {!live && showStart && (
            <form className="wow-start-form" onSubmit={(e) => { e.preventDefault(); onStart({ name, phone }); setShowStart(false); }}>
              <input className="wow-input" placeholder="Lead name" value={name} onChange={e=>setName(e.target.value)} required/>
              <div className="wow-input wow-input-phone">
                <span>+91</span>
                <input placeholder="98xxxxxxxx" value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,'').slice(0,10))} required/>
              </div>
              <button className="wow-cta" type="submit"><Icon name="phone" size={14}/>Dial</button>
